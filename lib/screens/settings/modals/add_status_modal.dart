...
class AddStatusModal extends StatefulWidget {
  ...
  final List<StatusModel>? existingStatuses; // To check for existing default

  const AddStatusModal({
    super.key,
    this.onStatusCreated,
    this.existingStatuses,
  });

  @override
  State<AddStatusModal> createState() => _AddStatusModalState();
}

class _AddStatusModalState extends State<AddStatusModal> {
  ...
  Future<void> _submit() async {
    ...
    // Check if setting as default and there's already a default status
    if (_isDefault && widget.existingStatuses != null && widget.existingStatuses!.isNotEmpty) {
      final existingDefault = widget.existingStatuses!.where((s) => s.isDefault).firstOrNull;
      ...
      if (existingDefault != null) {
        ...
        final confirmed = await showDialog<bool>(
          ...
          builder: (context) => AlertDialog(
            ...
            content: Text(
              '${localizations?.translate('setAsDefaultMessage') ?? 'There is already a default status'} (${existingDefault.name}). ${localizations?.translate('setAsDefaultMessage2') ?? 'Setting this status as default will unset the previous default. Do you want to continue?'}',
              textAlign: TextAlign.center,
            ),
              ...
              children: [
                ...
                Padding(
                  ...
                  child: Row(
                    ...
                    children: [
                      Text(
                        localizations?.translate('addStatus') ?? 'Add Status',
